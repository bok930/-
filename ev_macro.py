...
def _get_mss():
    global _MSS
    if _MSS is None and mss is not None:
        _MSS = mss.mss()
    return _MSS


def _load_templates():
    global _TPL_COLOR, _TPL_GRAY, _TPL_EDGE
    if _TPL_COLOR is not None:
        return
    if not TEMPLATE.exists():
        raise FileNotFoundError(f"템플릿 파일이 없습니다: {TEMPLATE}")
    _TPL_COLOR = cv2.imread(str(TEMPLATE), cv2.IMREAD_COLOR)
    if _TPL_COLOR is None:
        raise RuntimeError("지원신청 템플릿을 읽지 못했습니다.")
    _TPL_GRAY = cv2.cvtColor(_TPL_COLOR, cv2.COLOR_BGR2GRAY)
    _TPL_EDGE = cv2.Canny(_TPL_GRAY, 60, 160)


def _capture_roi_fast():
    """Capture only the lower-right action-button band. This is dramatically faster than full-screen capture."""
    sm = _get_mss()
    if sm is None:
        scr = np.array(pyautogui.screenshot())
        h, w = scr.shape[:2]
        x = int(w * 0.68); y = int(h * 0.82)
        roi = scr[y:h, x:w]
        return cv2.cvtColor(roi, cv2.COLOR_RGB2BGR), x, y
    mon = sm.monitors[0]
    sw, sh = mon['width'], mon['height']
    left = mon['left'] + int(sw * 0.68)
    top = mon['top'] + int(sh * 0.82)
    width = max(1, int(sw * 0.31))
    height = max(1, int(sh * 0.17))
    shot = np.array(sm.grab({'left': left, 'top': top, 'width': width, 'height': height}))
    return cv2.cvtColor(shot, cv2.COLOR_BGRA2BGR), left, top


def _blue_rect_candidates(roi):
    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
    # EV's blue action buttons. Keep this narrow to avoid expensive matching elsewhere.
    mask = cv2.inRange(hsv, np.array([92, 90, 120], np.uint8), np.array([122, 255, 255], np.uint8))
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    out = []
    for c in contours:
        x,y,w,h = cv2.boundingRect(c)
        if 45 <= w <= 180 and 20 <= h <= 45 and 1.5 <= w/float(h) <= 4.5:
            out.append((x,y,w,h))
    return out


def find_button(confidence=0.68):
    """Ultra-fast detection: only capture lower-right button band, then match blue candidates."""
    _load_templates()
    roi, ox, oy = _capture_roi_fast()
    th, tw = _TPL_COLOR.shape[:2]
    best = None

    # Candidate-first: usually 5-6 blue buttons, so only tiny ROIs are matched.
    for x,y,w,h in _blue_rect_candidates(roi):
        # candidate box can be slightly larger/smaller than template due to DPI
        x0=max(0,x-3); y0=max(0,y-3); x1=min(roi.shape[1],x+w+3); y1=min(roi.shape[0],y+h+3)
        cand=roi[y0:y1,x0:x1]
        if cand.shape[0] < 18 or cand.shape[1] < 35:
            continue
        # Resize template exactly to candidate dimensions. This avoids full-screen scale sweeps.
        rt=cv2.resize(_TPL_COLOR,(cand.shape[1],cand.shape[0]),interpolation=cv2.INTER_AREA if cand.shape[1] < tw else cv2.INTER_CUBIC)
        score_color=float(cv2.matchTemplate(cand,rt,cv2.TM_CCOEFF_NORMED)[0,0])
        cg=cv2.cvtColor(cand,cv2.COLOR_BGR2GRAY)
        rg=cv2.cvtColor(rt,cv2.COLOR_BGR2GRAY)
        score_gray=float(cv2.matchTemplate(cg,rg,cv2.TM_CCOEFF_NORMED)[0,0])
        score=0.65*score_color+0.35*score_gray
        if best is None or score>best[0]:
            best=(score,ox+x0,oy+y0,cand.shape[1],cand.shape[0],"ROI-blue")

    # Very small fallback: 3 scales only inside the same ROI, not the whole screen.
    if best is None or best[0] < confidence:
        for scale in (0.85,1.0,1.15):
            w=max(20,int(tw*scale)); h=max(12,int(th*scale))
            if w>=roi.shape[1] or h>=roi.shape[0]: continue
            rt=cv2.resize(_TPL_COLOR,(w,h),interpolation=cv2.INTER_AREA if scale<1 else cv2.INTER_CUBIC)
            g=cv2.cvtColor(roi,cv2.COLOR_BGR2GRAY); tg=cv2.cvtColor(rt,cv2.COLOR_BGR2GRAY)
            r=cv2.matchTemplate(g,tg,cv2.TM_CCOEFF_NORMED)
            _,val,_,loc=cv2.minMaxLoc(r)
            if best is None or float(val)>best[0]:
                best=(float(val),ox+loc[0],oy+loc[1],w,h,"ROI-scale")
            if val>=0.88: break
