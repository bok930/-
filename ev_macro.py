...
def _fast_blue_candidates(screen_bgr):
    """Find likely blue rounded buttons first; this is much faster than scanning many scales."""
    hsv = cv2.cvtColor(screen_bgr, cv2.COLOR_BGR2HSV)
    # Template's dominant blue is around H=104. Allow a fairly wide range for browser/display variations.
    mask = cv2.inRange(hsv, np.array([92, 90, 120], np.uint8), np.array([118, 255, 255], np.uint8))
    ...
    for c in contours:
        ...
        if 45 <= w <= min(260, sw) and 22 <= h <= min(70, sh):
            ratio = w / float(h)
            if 1.35 <= ratio <= 5.5:
                out.append((x, y, w, h))
    return out
...
def find_button(confidence=0.72):
    """Fast support-button detection: blue candidate scan first, template matching only on candidates."""
    ...
    screen = _capture_screen_fast()
    th, tw = tpl.shape[:2]
    best = None

    # 1) Very fast path: detect blue button-shaped regions, then compare only those small regions.
    candidates = _fast_blue_candidates(screen)
    for x, y, w, h in candidates:
        # Give a tiny margin for rounded corners / anti-aliasing.
        x0, y0 = max(0, x - 2), max(0, y - 2)
        x1, y1 = min(screen.shape[1], x + w + 2), min(screen.shape[0], y + h + 2)
        roi = screen[y0:y1, x0:x1]
        if roi.shape[0] < 15 or roi.shape[1] < 25:
            continue
        rt = cv2.resize(tpl, (roi.shape[1], roi.shape[0]), interpolation=cv2.INTER_AREA if roi.shape[1] < tw else cv2.INTER_CUBIC)
        val = float(cv2.matchTemplate(roi, rt, cv2.TM_CCOEFF_NORMED)[0, 0])
        # Also compare grayscale text/shape.
        rg = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        tg = cv2.cvtColor(rt, cv2.COLOR_BGR2GRAY)
        edge_val = float(cv2.matchTemplate(rg, tg, cv2.TM_CCOEFF_NORMED)[0, 0])
        score = 0.75 * val + 0.25 * edge_val
        cand = (score, x0, y0, roi.shape[1], roi.shape[0], "fast-blue")
        if best is None or cand[0] > best[0]:
            best = cand

    # 2) Fallback: a small number of scales over the full screen only if the fast path failed.
    if best is None or best[0] < confidence:
        ...
        for scale in scales:
            ...
            if w >= screen.shape[1] or h >= screen.shape[0]:
                continue
            rt = cv2.resize(tpl, (w, h), interpolation=cv2.INTER_AREA if scale < 1 else cv2.INTER_CUBIC)
            result = cv2.matchTemplate(screen, rt, cv2.TM_CCOEFF_NORMED)
            _, val, _, loc = cv2.minMaxLoc(result)
            cand = (float(val), loc[0], loc[1], w, h, "fallback")
            if best is None or cand[0] > best[0]:
                best = cand
            if val >= max(confidence + 0.10, 0.86):
                break
