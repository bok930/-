...
def get_ev_http_offset(samples=7):
    """Estimate EV web server clock offset from HTTP Date header."""
    session = requests.Session()
    values = []
    for i in range(samples):
        t0 = time.time()
        url = EV_URL + ("&" if "?" in EV_URL else "?") + f"_macro_ts={time.time_ns()}"
        try:
            r = session.get(url, timeout=5, headers={"Cache-Control": "no-cache", "Pragma": "no-cache"})
            t1 = time.time()
        except Exception:
            continue
    ...
    return sum(x[0] for x in best) / len(best), min(x[1] for x in best), "EV 웹서버 Date"


def ntp_offset(host, timeout=2.5):
    """SNTP offset using four timestamps."""
    ...
    try:
        ...
        t4 = time.time()
        if len(data) < 48:
            raise RuntimeError("NTP 응답이 너무 짧습니다.")
...
def get_server_offset():
    """Try EV web server first; fall back to reliable internet NTP time."""
    ev = get_ev_http_offset()
    if ev is not None:
        return ev
    ...
    for host in servers:
        try:
            ...
            results.append((off, rtt, host))
        except Exception:
            pass

    if not results:
        raise RuntimeError(
            "EV 웹서버 시간과 인터넷 표준시간 서버에 모두 연결하지 못했습니다.\n"
            "인터넷 연결 또는 회사/기관 방화벽에서 HTTPS/UDP 123 포트가 차단되어 있는지 확인해 주세요."
        )

    results.sort(key=lambda x: x[1])
    off, rtt, host = results[0]
...
class App:
    def __init__(self, root):
        ...
        ttk.Label(box2, text="① 시간 동기화\n② 목표시각까지 대기\n③ 정각에 현재 마우스 위치 클릭\n④ '지원신청' 이미지 자동 탐색 후 클릭").pack(anchor="w", padx=10, pady=8)

        match_row = ttk.Frame(box2)
        match_row.pack(fill="x", padx=10, pady=6)
        ttk.Label(match_row, text="이미지 매칭 기준").pack(side="left")
        ttk.Scale(match_row, from_=0.65, to=0.95, variable=self.conf, orient="horizontal", length=180).pack(side="left", padx=10)
        ttk.Label(match_row, text="권장 0.82").pack(side="left")
        ...
        # Global keyboard hook. Keep a strong reference so it is not garbage-collected.
    ...
    def sync(self):
        def work():
            try:
                self.log("시간 동기화 중... (EV 웹서버 우선, 실패 시 NTP)")
    ...
    def run(self, target_text):
        try:
            ...
            self.log(f"{target.strftime('%Y-%m-%d %H:%M:%S')}까지 대기 중...")
            while self.running:
            ...
            if not found:
                raise RuntimeError("'지원신청' 버튼을 찾지 못했습니다. 브라우저 확대율/화면 상태를 확인하세요.")
